...
async function compactEntityContext(repo){
  ...
  out.defaults={account_id:await repo.setting('default_account',''),today:parseDateInput('امروز'),yesterday:parseDateInput('دیروز'),currency:currencyCode(await repo.setting('default_currency',DEFAULT_CURRENCY))};return out;
}
...
export async function normalizeAiActions(repo,actions=[]){const out=[];for(const item of actions){const n=await normalizeAction(repo,item);if(n.clarify)return n;out.push(n.action);}return{actions:out};}
...
export async function handleAiText(repo,env,text,history=[]){
  ...
  if(plan.kind==='actions'){
    const normalized=await normalizeAiActions(repo,plan.actions);if(normalized.clarify)return{kind:'clarify',text:normalized.clarify,options:normalized.options||[]};
...
export async function confirmAiActions(repo,finance,draftId){
  ...
  const normalized=await normalizeAiActions(repo,actions);if(normalized.clarify){const e=new Error('AI_NEEDS_CLARIFICATION');e.detail=normalized.clarify;throw e;}
