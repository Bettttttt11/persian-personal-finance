...
async function ensureRef(repo,sheet,id){if(!id)return null;const row=await repo.getById(sheet,id);if(!row||bool(row.is_deleted))throw new Error('NOT_FOUND');return row;}
...
export class FinanceService{
  ...
  async createTransaction(rawInput,source='manual'){
    ...
    const inputCurrency=input.currency||await this.repo.setting('default_currency',DEFAULT_CURRENCY);
    const amount=toRial(intMoney(input.amount,false),inputCurrency),fee=toRial(intMoney(input.fee_amount||0,true),inputCurrency);
  ...
  async editTransaction(id,patch){
    ...
    const clean={...patch};
    const inputCurrency=clean.currency||await this.repo.setting('default_currency',DEFAULT_CURRENCY);
    if(clean.amount!==undefined)clean.amount=toRial(intMoney(clean.amount,false),inputCurrency);
  ...
  async accountBalances(accountIds=null){
    const accounts=await this.repo.list('Accounts',{limit:1000,includeDeleted:false});
  ...
  async applyRules(input){
    const rules=await this.repo.list('Rules',{limit:1000,filter:r=>!bool(r.is_deleted)&&bool(r.enabled)});rules.sort((a,b)=>Number(b.priority||0)-Number(a.priority||0));let out={...input};
